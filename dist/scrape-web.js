...
function generateFileNameFromHTML(html) {
...
const removeFooterFromHTML = (html) => {
    // Load the HTML content into Cheerio
    const $ = cheerio.load(html);
...
async function downloadPdfAndStore(filePath, url) {
    try {
...
async function htmlToMarkdown(html, filePath) {
    const turndownService = new TurndownService();
...
// 
export const scrapeWebToMarkdown = async (url) => {
    try {
        if (!existsSync('pdfs')) {
            mkdirSync('pdfs');
        }
...
//   });
//# sourceMappingURL=scrape-web.js.map