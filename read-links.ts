...
const readAllLinksFromFile = async (filePath: string) : Promise<string[]> => {
    try {
        // Read the file
        ...
        const allLinks = documents.flat().flatMap((doc: any) => doc.links);
        return allLinks;
    } catch (error) {
...
const fetchContentFromLinks = async (links: string[]) => {
