...
function createNotebooksDirectory(dataDir: string) {
...
async function convertNotebookToMarkdown(notebookPath: string) {
    try {
...
function extractLinksFromMarkdown(markdownPath: string): string[] {
...
const main = async () => {
    ...
    const ipynbLinks = await page.$$eval('a', (anchors) =>
        [...new Set(anchors
