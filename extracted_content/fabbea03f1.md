...
These components can be setup to share the same data model. Therefore you can do the following combinations:
  ...
  * Connect FITS viewer coverage image to a table
  * Connect XY plot to a table. A Table with any data and a XY Plot showing plots from any two columns of that table.
...
_return_ object viewer interface  
...
_return_ function a function that will remove the listener  
...
For the details of FITS plotting parameters see: fits-plotting-parameters.md
...
_return_ string a serialized version of range values to be passed as a viewer parameter  
...
extType string extension type, must be 'AREA_SELECT', 'LINE_SELECT', 'POINT', or 'CIRCLE_SELECT' (_details below_)  
callback function the function to call when the extension is selected (_details below_)  
...
extType details:
  ...
  * 'CIRCLE_SELECT' - When the user draws a circle (_not yet supported, coming soon_)



callback function takes one parameter, an object literal, the fields vary depend on the extension type - _todo - need to document callback object literal parameters_
...
_Example of two histogram traces:_
...
Documentation generated by JSDoc 3.5.5 on 2019-08-08T16:48:33-07:00 using the DocStrap template. 
